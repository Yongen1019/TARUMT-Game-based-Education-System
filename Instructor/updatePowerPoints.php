...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Update Power Points</title>
    ...
    <body>
        ...
        ?>
            ...
            <div class="contentMiddle">
                <h1>Update Power Exchange Points</h1>
                <div class="addClassFormContainer">
                    ...
                    if($findPowerResult){
                        while ($row = $findPowerResult->fetch_assoc()){
                            ...
                            if(isset($_POST["update"])){
                                $newPointExchange = $_POST['exchangePoints'];
                                
                                if($newPointExchange == null){
                                    echo"<script type='text/javascript'> 
                                    alert('Please enter a number in the Exchange Points field.')
                                    </script>";
                                }else{
                                    $updateQuery = "UPDATE GamePower SET pointExchange = '$newPointExchange' WHERE gameID = '$gameID' and powerID = '$powerID'";
                                    ...
                                    if($updateResult){
                                        ...
                                        alert('Exchange point is successfully updated.')
                                        </script>";
                                    }else{
                                        echo"<script type='text/javascript'> 
                                        location.replace('powerStore.php?instructorID={$instructorID}&classID={$classID}&gameID={$gameID}')
                                        alert('Failed to update power's exchange point.')
                                        </script>";
                                    }
                                }
                            }
                    ...
                    <form action="" method="post" class="addClassForm">
                        ...
                        <label for="className">Exchange Points</label><br>
                        <input type="number" name="exchangePoints" min="0" max="20" value="<?php echo $pointExchange; ?>" ><br>
                        
                        <input type="submit" name="update" value="UPDATE POINTS" class="updateBtn">
                    </form> 
                </div>
            </div>
        </div>
